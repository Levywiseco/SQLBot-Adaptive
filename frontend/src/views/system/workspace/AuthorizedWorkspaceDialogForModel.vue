<template>
  <el-dialog
    ...
    :title="$t('authorized_space.modify_authorized_space')"
...
import { workspaceList, workspaceModelMapping, workspaceModelMappingUpdate } from '@/api/workspace'
...
const open = async (id: any) => {
  ...
  isIndeterminate.value = false
  const [workspaceListResult, workspaceMappingResult] = await Promise.allSettled([
    workspaceList(),
    workspaceModelMapping(id),
  ])
  workspace.value =
    workspaceListResult.status === 'fulfilled'
      ? JSON.parse(JSON.stringify(workspaceListResult.value))
      : []
  checkedWorkspace.value =
